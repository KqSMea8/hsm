...
class Util(object):
    ...
    def get_key(key, dic, default=''):
        if (Util.is_list(key) and len(key) <= 1) or Util.is_str(key):
            if Util.is_list(key):
                key = str(key[0])
            if key in dic and not Util.is_none(dic[key]):
                return dic[key]
            else:
                return default
        else:
    ...
    @staticmethod
    def continue_browser_get(url, count, keyword=''):
        for i in range(count):
            res_str = Util.browser_get(url)
            if not res_str:
                continue
            if not keyword:
                try:
                    dict = Util.gzdecode(res_str)
                except Exception as e:
                    dict = res_str
            else:
                try:
                    data = json.loads(res_str)
                    dict = Util.get_key(keyword, data)
                except Exception as e:
                    dict = False
            if not dict:
                continue
    ...
    @staticmethod
    def get_unique_id(small, big=None):
        if big is not None:
            small = int(str(small) + '000000000000000')
            return small + int(big)
        else:
            return int(small)
