...
class MysqlTable(object):
    ...
    def make_select(select_list):
        if not Util.is_list(select_list):
            return '*'
        new_list = []
    ...
    def make_where_item(self, item):
        if not Util.is_list(item):
            item = [item, '=', 'and']
        else:
            length = len(item)
            if length < 2:
    ...
    def make_set(self, set_dict):
        set_str = ''
        for key in set_dict:
            val = set_dict[key]
            if Util.is_str(val):
                val = "'" + Util.addslashes(val) + "',"
            elif Util.is_int(val):
                val = str(val) + ','
            set_str += "`" + key + "`=" + val
        return set_str[:-1]
