...
App.prototype.loadModal = function (modal) {
    ...
    if (db.data.isIos) {
        if (modal === 'index') {
            ...
            if (!!this.stacks[this.stacks.length - 2] === false) this.doAnimation(null, this.getJqModal(this.stacks[this.stacks.length - 1]), false);else this.doAnimation(this.getJqModal(this.stacks[this.stacks.length - 2]), this.getJqModal(this.stacks[this.stacks.length - 1]), false);
            return;
        }
    }
    Util.hasLoadJsCallBack(modal + '.vue', function () {
        window[modal].vue.init();
        if (!!_this2.stacks[_this2.stacks.length - 2] === false) _this2.doAnimation(null, _this2.getJqModal(_this2.stacks[_this2.stacks.length - 1]), false);else _this2.doAnimation(_this2.getJqModal(_this2.stacks[_this2.stacks.length - 2]), _this2.getJqModal(_this2.stacks[_this2.stacks.length - 1]), false);
    });
};
